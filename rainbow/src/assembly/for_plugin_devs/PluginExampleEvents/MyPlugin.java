...
public class MyPlugin extends PluginBase
{
	...
	public void onStartup(MC_Server argServer)
	{
		...
		AsyncLog.StartAsyncLog();
		server = argServer;
		
		// Uncomment this for packet listener sample
		server.registerPlayerPacketListener(new MyPlayerPacketListener());
		server.registerServerPacketListener(new MyServerPacketListener());
	}
	public void onShutdown()
	{
		System.out.println("======= Event Sample --- Shutdown! =======");
		AsyncLog.EndAsyncLog();
	}
	...
	{
		String logMsg = String.format("%s LOGIN from IP %s. UUID=%s", playerName, ip, uuid.toString());
	...
	public void onInteracted(MC_Player plr, MC_Location loc, MC_ItemStack isHandItem)
	{
		...
		if((strHandItem != null) && (strHandItem.length() > 0)) logMsg += " w/" + strHandItem;
	...
	public void onBlockBroke(MC_Player plr, MC_Location loc, int blockKey)
	{
		...
		System.out.println("EventSamplePlugin[OLD] -- " + logMsg);
		AsyncLog.Log(logMsg);
	}
	...
	{
		String logMsg = String.format("%s Attempting Block Place or Interact @ %s, dir=%s", plr.getName(), loc.toString(), dir.toString());
		System.out.println("EventSamplePlugin -- " + logMsg);
	    ...
	    if(chest != null)
	    {
	    	...
	    	if(chest.GetLinkedChestAt(MC_DirectionNESWUD.NORTH) != null) strChestDirs.add("NORTH");
	    	if(chest.GetLinkedChestAt(MC_DirectionNESWUD.SOUTH) != null) strChestDirs.add("SOUTH");
	    	if(chest.GetLinkedChestAt(MC_DirectionNESWUD.EAST) != null) strChestDirs.add("EAST");
	    	if(chest.GetLinkedChestAt(MC_DirectionNESWUD.WEST) != null) strChestDirs.add("WEST");
	    	if(strChestDirs.size() > 0) System.out.println("* Chests at Directions: " + RainbowUtils.GetCommaList(strChestDirs));
	    	else System.out.println("* This chest is all alone! He needs a hug...");
		} // end of 'if chest'
	...
	public void onAttemptDamageHangingEntity(MC_Player plr, MC_Location loc, MC_HangingEntityType entType, MC_EventInfo ei)
	{
		...
		{
			if(plr != null) plr.sendMessage(ChatColor.RED + "You can't damage that entity!");
			ei.isCancelled = true;
	...
	public void onAttemptItemFrameInteract(MC_Player plr, MC_Location loc, MC_ItemFrameActionType actionType , MC_EventInfo ei)
	{
		...
		if(ToggleDamageHanging)
		{
			if(plr != null) plr.sendMessage(ChatColor.RED + "You can't operate on that item frame!");
			ei.isCancelled = true;
			System.out.println("Prevented: " + logMsg);
		}
	...
	// Use instead now: onAttemptEntityDamage	
	/*public void onAttemptPlayerTakeDamage(MC_Player plr, MC_DamageType dmgType, MC_EventInfo ei)
	{
		if(ei.isCancelled) return;
		
		String logMsg = String.format("%s Taking Damage Type %s", plr.getName(), dmgType.toString()); 
		System.out.println("EventSamplePlugin -- " + logMsg);
		
		if(TogglePlayerDamage)
		{
			plr.sendMessage(ChatColor.RED + "Prevented Damage Type: " + dmgType);
			ei.isCancelled = true;
			System.out.println("Prevented: " + logMsg);
		}
	}*/
	...
	public void onSignChanging(MC_Player plr, MC_Sign sign, MC_Location loc, List<String> newLines, MC_EventInfo ei) 
	{
		...
		else System.out.println("SIGN COORDINATES WRONG?  No Sign at location.");
	}
	...
	/*
	// Removed since this is destructive.  Use on a test world only!
	// You can generate custom terrain!
	public void onGenerateWorldColumn(int x, int z, MC_GeneratedColumn data) 
	{
		short[] arr = new short[256];
		for(int y=0; y<arr.length; y++)
		{
			if(y < 5) arr[y] = 7; // bedrock 
			else if(y < 10) arr[y] = 14; // gold ore 
			else if(y < 15) arr[y] = 15; // iron ore 
			else if(y < 20) arr[y] = 16; // coal ore 
			else if(y < 25) arr[y] = 21; // lapis ore 
			else if(y < 35) arr[y] = 0; // air pocket 
			else if(y < 50) arr[y] = 19; // sponge 
			else if(y < 55) arr[y] = 35; // wool 
			else if(y < 60) arr[y] = 56; // diamond ore
			else if(y < 61) arr[y] = 2; // grass
			else arr[y] = 0; // air
		}

		// Checkerboard diamond blocks on top, why not...
		if((x+z)%2 == 0) arr[64] = 57; // diamond block
		data.setData(arr);
	}
...
//			if(is == null) continue;
	...
	public void onServerFullyLoaded() 
	{
		...
		// Experiment with inter-plugin communication...
		// -----------------------------------------------------
		for(PluginInfo info : server.getPlugins())
		{
			// Find the other example plugin...
			if(info.ref instanceof PluginExample.MyPlugin)
			{
				// Get a reference
				PluginExample.MyPlugin rac = (PluginExample.MyPlugin)info.ref;
				
				// Report something about a variable in that plugin...
				System.out.println("MapSize from PluginExample: " + rac.lastInfoMap.size());
				
				// Now for Billy G: Test loading a resource that's in both this plugin and other one.
				// Prior to V37 Rainbow this would get the wrong resource.  V37+ it gets
				// the test.txt inside the calling plugin (PluginExampleEvents)
				try
				{
					InputStream fis = MyPlugin.class.getResourceAsStream("/test.txt");
	...
	{
		String strKiller = "?";
		if(entKiller != null) strKiller = String.format("%s (%s)", entKiller.getName(), entKiller.getType());
