...
public class MiscUtils
{
	...
	public static boolean TooSoon(MC_Player plr, String what, int seconds) {
		if(plr.isOp()) return false;

		String key = what + "." + plr.getName();

		Long msBefore = tooSoon.get(key);
		Long curMS = System.currentTimeMillis();
		if (msBefore != null) {
			Long msDelta = (curMS - msBefore);
			// Long msWaitTime = 1000L*60*60; // 60 minutes
			Long msWaitTime = 1000L * seconds;
			if (msDelta < msWaitTime) {
				plr.sendMessage(ChatColor.RED + "[" + what + "] Too soon, you must wait: " + ChatColor.AQUA + RainbowUtils.TimeDeltaString_JustMinutesSecs(msWaitTime - msDelta));
				return true;
			}
		}
